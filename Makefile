...
install-server-package: uninstall-server-package
	$(PYTHON_PIP) install --force-reinstall --no-deps "$(shell find dist -name "odh_elyra-*-py3-none-any.whl")"
...
validate-runtime-image: # Validate that runtime image meets minimum criteria
	...
	for cmd in $$required_commands ; do \
		...
		if [ $$cmd == "python3" ]; then \
			...
			if [[ $$IMAGE_PYTHON3_MINOR_VERSION -lt 11 ]]; then \
				echo "WARNING: Container image $$image requires Python 3.11 or greater for latest generic component dependency installation" ; \
				fail=1; \
			elif [[ $$IMAGE_PYTHON3_MINOR_VERSION -ge 11 ]]; then \
